...
import {
  ...
  useDeletePlotJournalEntryMutation,
} from "./plot-journal.hooks";
import { PlotJournalEntryScreenProps } from "./navigation/plots-routes";
...
export function PlotJournalEntryScreen({
  route,
  navigation,
  ...
  function handleDelete() {
    Alert.alert(t("animals.delete_journal_entry"), undefined, [
      { text: t("buttons.cancel"), style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: async () => {
          await deleteMutation.mutateAsync({ entryId, plotId });
          navigation.goBack();
        },
      },
    ]);
    ...
    <ContentView headerVisible>
      <ScrollView showHeaderOnScroll headerTitleOnScroll={entry.title}>
        ...
        {entry.images.length > 0 && (
          ...
          >
            {entry.images.map((item: PlotJournalImage) => {
