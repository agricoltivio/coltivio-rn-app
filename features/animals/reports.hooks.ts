...
export function useDownloadAnimalsReportMutation(
  onSuccess?: () => void,
  onError?: (error: Error) => void,
) {
  const api = useApi();
  return useMutation({
    mutationFn: async (input: DownloadAnimalsReportInput) => {
      const { base64, fileName } =
        await api.reports.downloadAnimalsReport(input);
      const file = new File(Paths.cache, fileName);
      file.write(base64, { encoding: "base64" });
      await shareAsync(file.uri, {
        mimeType:
