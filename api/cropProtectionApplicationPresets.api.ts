...
export type CropProtectionApplicationPresetMethod = CropProtectionApplicationPreset["method"];
export type CropProtectionApplicationPresetUnit = CropProtectionApplicationPreset["unit"];
...
export function cropProtectionApplicationPresetsApi(client: FetchClient) {
  return {
    async getCropProtectionApplicationPresets(): Promise<CropProtectionApplicationPreset[]> {
      const { data } = await client.GET("/v1/cropProtectionApplications/presets", {});
      return data!.data.result;
    },

    async getCropProtectionApplicationPresetById(presetId: string): Promise<CropProtectionApplicationPreset> {
    ...
    async createCropProtectionApplicationPreset(
      ...
      const { data } = await client.POST("/v1/cropProtectionApplications/presets", {
        body: preset,
      });
      return data!.data;
    },
    ...
    async deleteCropProtectionApplicationPreset(presetId: string): Promise<void> {
      await client.DELETE("/v1/cropProtectionApplications/presets/byId/{presetId}", {
        params: { path: { presetId } },
      });
    },
  };
